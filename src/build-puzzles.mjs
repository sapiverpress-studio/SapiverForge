...
function hubPage() {
  const puzzleCards = config.puzzles.map((item) => `<article class="card"><h3>${esc(item.name)}</h3><p>${esc(item.description || "Choose a puzzle and use the variant rules to complete the grid.")}</p><a class="pill-button" href="/play/${esc(item.slug)}/">Play ${esc(item.name)}</a></article>`).join("\n");
  const gameCards = (config.otherGames || []).map((item) => `<article class="card"><h3>${esc(item.name)}</h3><p>${esc(item.description || "Play a Sapiver browser game.")}</p><a class="pill-button" href="/play/${esc(item.slug)}/">Play ${esc(item.name)}</a></article>`).join("\n");
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sapiver Puzzles | Daily Sudoku variants and browser games</title><meta name="description" content="Play Sapiver Puzzles: daily Sudoku variants, Trigoku and Sapiver browser games."><link rel="canonical" href="https://suite.sapiverpress.co.uk/puzzles/"><meta name="theme-color" content="${BRAND.rail}"><style>${shellStyles()}</style></head><body>${header()}<main><section class="hero"><p class="eyebrow">Sapiver Puzzles</p><h1>One place for the puzzles.</h1><p class="lead">Choose from daily Sudoku variants, Trigoku and other Sapiver browser games. Pick a puzzle, use the variant rules and start solving.</p><div class="actions"><a class="pill-button" href="#sudoku">Browse Sudoku variants</a>${gameCards ? '<a class="pill-button secondary" href="#games">Other games</a>' : ""}</div></section><div class="section-head" id="sudoku"><div><p class="eyebrow">Daily play</p><h2>Sudoku variants & Trigoku</h2></div><p>${config.puzzles.length} playable games</p></div><section class="grid" aria-label="Sudoku variants">${puzzleCards}</section>${gameCards ? `<div class="section-head" id="games"><div><p class="eyebrow">More Sapiver games</p><h2>Other browser games</h2></div></div><section class="grid" aria-label="Other games">${gameCards}</section>` : ""}</main><footer>Sapiver Puzzles is part of <a href="/">Sapiver Press</a>.</footer></body></html>`;
}
...
function patchHomepage() {
  ...
  const block = `${start}<section class="posts" id="sapiver-puzzles"><p class="eyebrow">Sapiver Puzzles</p><h2>Daily Sudoku variants and browser games.</h2><p>Play Kropki, Killer, Thermo, Sudoku X, Trigoku and more from one Sapiver Puzzles hub.</p><p><a class="button" href="/puzzles/">Play Sapiver Puzzles</a></p></section>${end}`;
...
patchSiteNavigation();
patchHomepage();
console.log(`Sapiver Puzzles built: ${config.puzzles.length} puzzle games + ${(config.otherGames || []).length} other games; routes and branding checked.`);
