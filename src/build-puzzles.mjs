...
function shellStyles() {
  return `
    :root{--ink:${BRAND.ink};--ink-deep:${BRAND.inkDeep};--muted:${BRAND.muted};--paper:${BRAND.paper};--paper-soft:${BRAND.paperSoft};--panel:${BRAND.panel};--line:${BRAND.line};--accent:${BRAND.accent};--accent-deep:${BRAND.accentDeep};--gold:${BRAND.gold};--rail:${BRAND.rail};--rail-soft:${BRAND.railSoft}}
...
function header(active = "puzzles") {
  return `<header class="site-header"><a class="brand" href="/">Sapiver Press<small>Practical tools, publishing and learning</small></a><nav aria-label="Site links"><a href="/">Home</a><a href="/puzzles/"${active === "puzzles" ? ' aria-current="page"' : ""}>Sapiver Puzzles</a><a href="/learn/">Sapiver Learn</a><a href="/parents/">AI Inquisitive Parents</a><a href="/daily-brief/">Daily Brief</a></nav></header>`;
}

function hubPage() {
  const puzzleCards = config.puzzles.map((item) => `<article class="card"><h3>${esc(item.name)}</h3><p>Daily browser puzzle. Progress is kept by the game on this device.</p><a class="pill-button" href="/play/${esc(item.slug)}/">Play ${esc(item.name)}</a></article>`).join("\n");
  ...
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sapiver Puzzles | Daily Sudoku variants and browser games</title><meta name="description" content="Play Sapiver Puzzles: daily Sudoku variants, Trigoku and Sapiver browser games."><link rel="canonical" href="https://suite.sapiverpress.co.uk/puzzles/"><meta name="theme-color" content="${BRAND.rail}"><style>${shellStyles()}</style></head><body>${header()}<main><section class="hero"><p class="eyebrow">Sapiver Puzzles</p><h1>One place for the puzzles.</h1><p class="lead">The Sapiver Press browser puzzle collection: daily Sudoku variants, Trigoku and other games. The original game engines are preserved while the collection is presented in the current Sapiver corporate colours.</p><div class="actions"><a class="pill-button" href="#sudoku">Browse Sudoku variants</a>${gameCards ? '<a class="pill-button secondary" href="#games">Other games</a>' : ""}</div></section><div class="section-head" id="sudoku"><div><p class="eyebrow">Daily play</p><h2>Sudoku variants & Trigoku</h2></div><p>${config.puzzles.length} playable games</p></div><section class="grid" aria-label="Sudoku variants">${puzzleCards}</section>${gameCards ? `<div class="section-head" id="games"><div><p class="eyebrow">More Sapiver games</p><h2>Other browser games</h2></div></div><section class="grid" aria-label="Other games">${gameCards}</section>` : ""}<aside class="notice"><strong>Built to stay put.</strong> This catalogue and its historic play URLs are regenerated from source-controlled configuration whenever the suite is published.</aside></main><footer>Sapiver Puzzles is part of <a href="/">Sapiver Press</a>. Individual puzzle engines remain independently hosted so an update to the main suite cannot erase the games themselves.</footer></body></html>`;
}

function playPage() {
  const allItems = [...config.puzzles, ...(config.otherGames || [])];
  const lookup = {};
  ...
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>Sapiver Puzzles | Play</title><meta name="theme-color" content="${BRAND.rail}"><style>${shellStyles()} body{overflow:hidden}.play-wrap{height:100dvh;display:grid;grid-template-rows:auto 1fr}.site-header{width:calc(100% - 16px);margin:8px auto 0;padding:10px 14px;border-radius:17px}.site-header .brand{font-size:1.05rem}.site-header .brand small{display:none}.site-header nav a{min-height:34px;padding:6px 10px;font-size:.83rem}.game-stage{width:calc(100% - 16px);height:calc(100% - 8px);margin:8px auto 0;background:#fff;border:1px solid rgba(185,155,94,.38);border-radius:18px 18px 0 0;overflow:hidden;box-shadow:0 8px 26px rgba(20,49,42,.11);position:relative}.game-stage iframe{width:100%;height:100%;border:0;display:block;background:#fff}.loading{position:absolute;inset:0;display:grid;place-items:center;background:var(--paper-soft);color:var(--muted);z-index:0}.game-stage iframe{position:relative;z-index:1}@media(max-width:680px){.site-header nav a:not([aria-current]){display:none}.site-header{flex-wrap:nowrap}.game-stage{border-radius:14px 14px 0 0}}</style></head><body><div class="play-wrap">${header()}<main class="game-stage" id="stage"><div class="loading" id="loading">Loading Sapiver puzzle…</div></main></div><script>(()=>{const lookup=${lookupJson};const parts=location.pathname.split('/').filter(Boolean);const slug=decodeURIComponent(parts[parts.length-1]||'');const item=lookup[slug];if(!item){location.replace('/puzzles/');return}const source=new URL(item.source);for(const [key,value] of new URLSearchParams(location.search))source.searchParams.append(key,value);if(new URLSearchParams(location.search).has('sapiver_export')){location.replace(source.href);return}document.title=item.name+' | Sapiver Puzzles';const stage=document.getElementById('stage');const frame=document.createElement('iframe');frame.src=source.href;frame.title=item.name;frame.allow='clipboard-write; fullscreen';frame.referrerPolicy='strict-origin-when-cross-origin';frame.addEventListener('load',()=>document.getElementById('loading')?.remove());stage.append(frame)})();</script></body></html>`;
}
...
patchSiteNavigation();
patchHomepage();
console.log(`Sapiver Puzzles built: ${config.puzzles.length} puzzle games + ${(config.otherGames || []).length} other games.`);
