...
function hubPage() {
  ...
  const gameCards = (config.otherGames || []).map((item) => `<article class="card"><h3>${esc(item.name)}</h3><p>${esc(item.description || "Play a Sapiver browser game.")}</p><a class="pill-button" href="${esc(item.source)}">Play ${esc(item.name)}</a></article>`).join("\n");
